...
function ShopOwnerMainPage() {
  // Refs for scrolling and section references
  const salesOverviewRef = useRef(null);
  const productsContainerRef = useRef(null);
  const ordersContainerRef = useRef(null);

  // State variables for products, orders, graph data, and user info
  ...
  // Scroll to Sales Overview section if hash is in URL
  useEffect(() => {
    if (
  ...
  // Fetch user info for the logged-in business owner
  useEffect(() => {
  ...
  const handleSelectOrder = (orderNumber) => {
    navigate(`/ShopOwnerOrdersPage?orderNumber=${orderNumber}`);
  };
