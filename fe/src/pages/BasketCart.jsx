...
const BasketCart = () => {
  // State for cart items, loading indicator, error messages, customer information, PayPal emails, shop owner information, and groups that have been paid
  ...
  // Fetch shop owner info based on their username
  ...
  // Group items by shop owner to manage separate orders per owner
  ...
  // Update the quantity of a cart item and save to the server
  ...
  // Remove an item from the cart and update on the server
  ...
  // Prepare orders for checkout, organizing by shop owner
  ...
  // Helper to render PayPalCheckoutButton component with necessary props
  const temp = (
    totalAmountForGroup,
    paypalEmail,
    merchantId,
  ...
  // Function to place an order, sending customer info and cart items to the server
  ...
  // Memoized checkout groups to prevent unnecessary re-renders
      ...
      <main className="basketCart-container">
        ...
        <div className="basket-cart-container">
          {cartItems.length === 0 ? (
            ...
            // Map through orders to create checkout sections per shop owner
            orders.map((order) => {
              ...
              const merchantId = ownerInfo.merchantId || null;
